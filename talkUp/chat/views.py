...
from .models import Usuario, Perfil, Publicacao, Comentario, Mensagem
from .serializers import UsuarioSerializer, PerfilSerializer, PublicacaoSerializer, ComentarioSerializer, MensagemSerializer
...
class MensagemEnviadaListAPIView(ListAPIView):
    serializer_class = MensagemSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return Mensagem.objects.filter(remetente=self.request.user)
    
    def get_serializer_context(self):
        context = super().get_serializer_context()
        context['timezone'] = pytz_timezone('America/Sao_Paulo')
        return context

class MensagemRecebidaListAPIView(ListAPIView):
    serializer_class = MensagemSerializer
    permission_classes = [IsAuthenticated]
...
class MensagemCreateAPIView(CreateAPIView):
    queryset = Mensagem.objects.all()
    serializer_class = MensagemSerializer
    permission_classes = [IsAuthenticated]

    def perform_create(self, serializer):
        serializer.save(destinatario_id=self.request.data['destinatario_id'])
